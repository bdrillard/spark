...
case class SampleExec(
  ...
  override def doConsume(ctx: CodegenContext, input: Seq[ExprCode], row: ExprCode): String = {
    ...
    if (withReplacement) {
      ...
      s"""
         | int $samplingCount = $samplerAccessor.sample();
      ...
      s"""
         | if ($samplerAccessor.sample() == 0) continue;
         | $numOutput.add(1);
         | ${consume(ctx, input)}
       """.stripMargin.trim
    }
...
case class RangeExec(range: org.apache.spark.sql.catalyst.plans.logical.Range)
  ...
  protected override def doProduce(ctx: CodegenContext): String = {
    ...
    val initTermAccessor = ctx.addMutableState("boolean", initTerm, s"$initTerm = false;")
    val number = ctx.freshName("number")
    val numberAccessor = ctx.addMutableState("long", number, s"$number = 0L;")
    ...
    val taskContextAccessor =
      ctx.addMutableState("TaskContext", taskContext, s"$taskContext = TaskContext.get();")
    val inputMetrics = ctx.freshName("inputMetrics")
    val inputMetricsAccessor = ctx.addMutableState("InputMetrics", inputMetrics,
        s"$inputMetrics = $taskContextAccessor.taskMetrics().inputMetrics();")
    ...
    val initRangeFunc = ctx.addNewFunction("initRange",
      s"""
        ...
        |     $numberAccessor = Long.MAX_VALUE;
        |   } else if (st.compareTo($BigInt.valueOf(Long.MIN_VALUE)) < 0) {
        |     $numberAccessor = Long.MIN_VALUE;
        |   } else {
        |     $numberAccessor = st.longValue();
        |   }
        |   $batchEndAccessor = $numberAccessor;
        |
        ...
        |     $BigInt.valueOf($numberAccessor));
        |   $numElementsTodoAccessor  = startToEnd.divide(step).longValue();
        |   if ($numElementsTodoAccessor < 0) {
        |     $numElementsTodoAccessor = 0;
        |   } else if (startToEnd.remainder(step).compareTo($BigInt.valueOf(0L)) != 0) {
        |     $numElementsTodoAccessor++;
        |   }
        | }
       """.stripMargin)
    ...
    s"""
      | // initialize Range
      | if (!$initTermAccessor) {
      |   $initTermAccessor = true;
      |   $initRangeFunc(partitionIndex);
      | }
      |
      | while (true) {
      |   while ($numberAccessor != $batchEndAccessor) {
      |     long $value = $numberAccessor;
      |     $numberAccessor += ${step}L;
      |     ${consume(ctx, Seq(ev))}
      |     if (shouldStop()) return;
      |   }
      |
      |   if ($taskContextAccessor.isInterrupted()) {
      |     throw new TaskKilledException();
      |   }
      |
      |   long $nextBatchTodo;
      |   if ($numElementsTodoAccessor > ${batchSize}L) {
      |     $nextBatchTodo = ${batchSize}L;
      |     $numElementsTodoAccessor -= ${batchSize}L;
      |   } else {
      |     $nextBatchTodo = $numElementsTodoAccessor;
      |     $numElementsTodoAccessor = 0;
      |     if ($nextBatchTodo == 0) break;
      |   }
      |   $numOutput.add($nextBatchTodo);
      |   $inputMetricsAccessor.incRecordsRead($nextBatchTodo);
      |
      |   $batchEndAccessor += $nextBatchTodo * ${step}L;
      | }
     """.stripMargin
  }
